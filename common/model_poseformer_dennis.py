...
# class Block(nn.Module): #Encoder Pre-LN
# 
#     def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
#                   drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm):
#         super().__init__()
#         self.norm1 = norm_layer(dim) #Layer Norm
#         self.norm1_2 = norm_layer(dim) #Layer Norm
...
# 
#     def forward(self, x):
#         x = x + self.drop_path(self.norm1_2(self.attn(self.norm1(x))))
#         x = x + self.drop_path(self.norm2_2(self.mlp(self.norm2(x))))
#         return x
         
class Block(nn.Module): #Encoder Post-LN
     ...
     def forward(self, x, prev=None):
         ...
         residual = x
         x = self.mlp(x)
         x = residual + self.drop_path(self.norm2(x))
         return x ,prev         


class PoseTransformer(nn.Module):
    def __init__(self, num_frame=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=20,
