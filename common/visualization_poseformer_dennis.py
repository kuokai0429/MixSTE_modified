...
#        ##### 畫可視化用 coco 轉 h36m 時
        ...
        joints_right_2d = [1, 2, 3, 14, 15, 16]                          # 依照detection的方法找出的2維點 找出其對稱點 並分配顏色
        colors_2d = np.full(keypoints.shape[1], 'black')
        colors_2d[joints_right_2d] = 'red'
        
#        #####
        
        ##### 畫可視化用 coco 轉 h36m 時                       用來跑沒有腳的   畫人身上的線用的
        
        #joints_right_2d = [1,2,3,14, 15, 16]                          # 依照detection的方法找出的2維點 找出其對稱點 並分配顏色
        #joints_left_2d =[7,8,9,10,11,12,13]
        #colors_2d = np.full(keypoints.shape[1], 'black') 
        #colors_2d[joints_left_2d] = 'black'
        #colors_2d[joints_right_2d] = 'red'
        
        #####
        
        if not initialized:
            image = ax_in.imshow(all_frames[i], aspect='equal')  # asepect = 'equal' 確保為正方形
            ...
            for j, j_parent in enumerate(parents):
                ...
                if len(parents) == keypoints.shape[1] and keypoints_metadata['layout_name'] != 'coco' :
                    ...
                    lines.append(ax_in.plot([keypoints[i, j, 0], keypoints[i, j_parent, 0]],
                                            [keypoints[i, j, 1], keypoints[i, j_parent, 1]], color=colors_2d))
                ...
                for n, ax in enumerate(ax_3d):
                    ...
                    #if (j == 1 or j ==2 or j==3 or j==4 or j==5 or j== 6 or j== 0):   # 一開始選擇哪幾個關節不要畫
                    #    lines_3d[n].append(0)
                    #else:
                    #    lines_3d[n].append(ax.plot([pos[j, 0], pos[j_parent, 0]],
                    #                           [pos[j, 1], pos[j_parent, 1]],
            ...
            for j, j_parent in enumerate(parents):
                ...
                #if j_parent == -1 or (j == 1 or j ==2 or j==3 or j==4 or j==5 or j== 6 or j== 0):   # 哪幾個關節更新的時候不要畫
                #    continue
                if j_parent == -1:
                    continue    
